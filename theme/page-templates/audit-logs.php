...
<div class="space-y-6" x-data="auditLogsPage()" x-init="init()">
	...
	<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
		...
		<div class="relative" x-data="{ exportOpen: false }">
			...
			>
				...
				<svg class="w-4 h-4 transition-transform" :class="{ 'rotate-180': exportOpen }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
	...
	<div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
		<!-- Today's Actions -->
		<div class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-lg border border-gray-200/50 dark:border-gray-700/50 p-5">
		...
		<template x-for="cat in statsCategories" :key="cat.key">
			<div class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-lg border border-gray-200/50 dark:border-gray-700/50 p-5">
				<div class="flex items-center gap-2">
					...
					<p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider" x-text="cat.label"></p>
				</div>
	...
	<!-- ═══════════════════════════════════════════════════════════════════
	     FILTERS & SEARCH
	     ═══════════════════════════════════════════════════════════════════ -->
	<div class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-lg border border-gray-200/50 dark:border-gray-700/50 p-4">
		<div class="flex flex-col lg:flex-row gap-3">
			...
			<div class="relative" x-data="{ userDropdownOpen: false }">
				...
				<div
					x-show="userDropdownOpen && userSuggestions.length > 0"
					x-cloak
				...
				>
					<template x-for="user in userSuggestions" :key="user.id">
						<button
							@click="selectUser(user); userDropdownOpen = false"
	...
	<div class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-lg border border-gray-200/50 dark:border-gray-700/50 overflow-hidden">
		...
		<div x-show="loading" class="p-8 text-center">
			...
			<p class="mt-2 text-sm text-gray-500 dark:text-gray-400"><?php esc_html_e( 'Loading audit logs...', 'vms-theme' ); ?></p>
		</div>
			...
			<table class="w-full">
				<thead>
					<tr class="border-b border-gray-200 dark:border-gray-700">
						<th class="text-left px-4 py-3 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider w-8"></th>
						...
						<th class="text-left px-4 py-3 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider hidden md:table-cell">
						...
						<th class="text-left px-4 py-3 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider hidden lg:table-cell">
							<?php esc_html_e( 'Entity', 'vms-theme' ); ?>
						</th>
						<th class="text-left px-4 py-3 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider hidden xl:table-cell">
						...
						<template x-if="true">
							<tbody>
								...
								>
									...
									<td class="px-4 py-3">
										<svg
											class="w-4 h-4 text-gray-400 group-hover:text-[var(--vms-primary)] transition-all duration-200"
											:class="{ 'rotate-90': expandedRows[row.id] }"
									...
									<!-- Date/Time -->
									<td class="px-4 py-3">
										<div class="text-sm text-gray-900 dark:text-white" x-text="formatDate(row.created_at)"></div>
										<div class="text-xs text-gray-500 dark:text-gray-400" x-text="formatTime(row.created_at)"></div>
									</td>
									...
									<td class="px-4 py-3">
										<div class="flex items-center gap-2">
											...
											<span class="text-sm font-medium text-gray-900 dark:text-white truncate max-w-[120px]" x-text="row.display_name || '<?php echo esc_js( __( 'System', 'vms-theme' ) ); ?>'"></span>
										</div>
									</td>

									<!-- Role -->
									<td class="px-4 py-3 hidden md:table-cell">
										<span class="text-xs text-gray-500 dark:text-gray-400 capitalize" x-text="formatRole(row.user_role)"></span>
									</td>
									...
									<td class="px-4 py-3 hidden xl:table-cell">
										<span class="text-xs font-mono text-gray-500 dark:text-gray-400" x-text="row.ip_address || '---'"></span>
									</td>
								</tr>
									...
									<td colspan="8" class="px-4 py-0">
										...
										>
											<div class="py-4 space-y-4">
												<!-- Detail Grid -->
												<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 p-4 bg-gray-50 dark:bg-gray-900/40 rounded-xl border border-gray-200/50 dark:border-gray-700/50">
													<!-- Entity Info -->
													<div>
														...
														<p class="text-sm text-gray-900 dark:text-white">
															...
															<span class="text-gray-400" x-show="row.entity_id"> #<span x-text="row.entity_id"></span></span>
														</p>
													</div>
													<!-- User Agent -->
													<div class="lg:col-span-2">
														...
														<p class="text-xs text-gray-600 dark:text-gray-400 break-all line-clamp-2" x-text="row.user_agent || '---'"></p>
													</div>
													<!-- IP -->
													<div>
														<p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase mb-1">
														...
														<p class="text-sm font-mono text-gray-900 dark:text-white" x-text="row.ip_address || '---'"></p>
													</div>
												</div>
												...
												<div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
													<!-- Old Values -->
													<div x-show="row.old_values && row.old_values !== 'null' && row.old_values !== '{}'" x-cloak>
														<div class="rounded-xl border border-red-200/50 dark:border-red-800/30 overflow-hidden">
															<div class="px-3 py-2 bg-red-50 dark:bg-red-900/20 border-b border-red-200/50 dark:border-red-800/30">
																<p class="text-xs font-semibold text-red-700 dark:text-red-400 uppercase tracking-wider">
																	<?php esc_html_e( 'Previous Values', 'vms-theme' ); ?>
																</p>
															</div>
															<pre class="p-3 text-xs font-mono text-gray-800 dark:text-gray-200 bg-white dark:bg-gray-900/60 overflow-x-auto max-h-48" x-text="formatJson(row.old_values)"></pre>
														</div>
													</div>

													<!-- New Values -->
													<div x-show="row.new_values && row.new_values !== 'null' && row.new_values !== '{}'" x-cloak>
														<div class="rounded-xl border border-green-200/50 dark:border-green-800/30 overflow-hidden">
															<div class="px-3 py-2 bg-green-50 dark:bg-green-900/20 border-b border-green-200/50 dark:border-green-800/30">
															...
															<pre class="p-3 text-xs font-mono text-gray-800 dark:text-gray-200 bg-white dark:bg-gray-900/60 overflow-x-auto max-h-48" x-text="formatJson(row.new_values)"></pre>
														</div>
													</div>

													<!-- Metadata -->
													<div x-show="row.metadata && row.metadata !== 'null' && row.metadata !== '{}'" x-cloak>
														<div class="rounded-xl border border-blue-200/50 dark:border-blue-800/30 overflow-hidden">
															<div class="px-3 py-2 bg-blue-50 dark:bg-blue-900/20 border-b border-blue-200/50 dark:border-blue-800/30">
																<p class="text-xs font-semibold text-blue-700 dark:text-blue-400 uppercase tracking-wider">
																	<?php esc_html_e( 'Metadata', 'vms-theme' ); ?>
																</p>
															</div>
															<pre class="p-3 text-xs font-mono text-gray-800 dark:text-gray-200 bg-white dark:bg-gray-900/60 overflow-x-auto max-h-48" x-text="formatJson(row.metadata)"></pre>
														</div>
													</div>
												</div>
		...
		<div x-show="!loading && rows.length === 0" class="p-12 text-center">
			...
			<p class="text-gray-500 dark:text-gray-400" x-text="hasActiveFilters ? '<?php echo esc_js( __( 'No logs match your current filters. Try adjusting your search criteria.', 'vms-theme' ) ); ?>' : '<?php echo esc_js( __( 'System activity will appear here as actions are performed.', 'vms-theme' ) ); ?>'"></p>
		</div>
		...
		<div x-show="!loading && rows.length > 0"
			...
			<div class="flex items-center gap-3 text-sm text-gray-500 dark:text-gray-400">
				<span>
					...
					-
					<span class="font-medium text-gray-900 dark:text-white" x-text="Math.min(currentPage * perPage, totalRows)"></span>
			...
			<div class="flex items-center gap-1">
				...
				<template x-for="page in visiblePages" :key="page">
					...
					<span
						x-show="page === '...'"
						class="w-9 h-9 flex items-center justify-center text-sm text-gray-400 dark:text-gray-500"
					>...</span>
				</template>
	...
	<!-- ═══════════════════════════════════════════════════════════════════
	     ...
	     ═══════════════════════════════════════════════════════════════════ -->
	<div
		x-show="showUserExportModal"
		x-cloak
		class="fixed inset-0 z-[200] flex items-center justify-center p-4"
	>
		<div @click="showUserExportModal = false" class="absolute inset-0 bg-black/50 backdrop-blur-sm"></div>
		<div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 w-full max-w-md" @click.stop>
			<div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
				<h3 class="text-lg font-semibold text-gray-900 dark:text-white">
					<?php esc_html_e( 'Export Logs for User/Guest', 'vms-theme' ); ?>
				</h3>
				<button @click="showUserExportModal = false" class="p-1 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700">
			...
			<div class="p-6 space-y-4">
				...
				<div class="relative" x-data="{ exportUserDropdownOpen: false }">
					...
					<input
						type="text"
						x-model="exportUserSearchTerm"
						@input.debounce.300ms="searchExportUsers()"
						@focus="exportUserDropdownOpen = true"
						@click.outside="exportUserDropdownOpen = false"
					...
					<!-- Suggestions -->
					<div
						x-show="exportUserDropdownOpen && exportUserSuggestions.length > 0"
						x-cloak
						class="absolute left-0 right-0 mt-1 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 max-h-48 overflow-y-auto z-50"
					>
						<template x-for="user in exportUserSuggestions" :key="user.id">
							<button
								@click="selectExportUser(user); exportUserDropdownOpen = false"
								class="flex items-center gap-2 w-full px-3 py-2 text-sm text-left text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors"
							>
								<div class="w-6 h-6 rounded-full bg-[var(--vms-primary)]/10 flex items-center justify-center text-xs font-medium text-[var(--vms-primary)]"
									x-text="user.display_name.charAt(0).toUpperCase()">
				...
				<div x-show="exportSelectedUser" x-cloak
					...
					<div>
						<p class="text-sm font-medium text-gray-900 dark:text-white" x-text="exportSelectedUser?.display_name"></p>
						<p class="text-xs text-gray-500 dark:text-gray-400">ID: <span x-text="exportSelectedUser?.id"></span></p>
					</div>
				...
				<div class="flex justify-end gap-3 pt-2">
					...
					<button
						@click="exportUserPdf()"
						:disabled="!exportSelectedUser || exporting"
...
function auditLogsPage() {
	return {
			...
			try {
				...
				if (data.success) {
					this.userSuggestions = (data.data.results || []).map(u => ({
						id: u.id || u.user_id,
						display_name: [u.first_name, u.last_name].filter(Boolean).join(' ') || u.display_name || '<?php echo esc_js( __( 'Unknown', 'vms-theme' ) ); ?>'
					}));
				}
			} catch (e) {
		...
		getActionBadgeClass(action) {
			...
			return map[action] || 'bg-gray-50 text-gray-700 dark:bg-gray-900/20 dark:text-gray-400 ring-1 ring-gray-600/10';
		},
		...
		async doExport(params) {
			...
			try {
				...
				if (contentType && contentType.includes('application/pdf')) {
					...
					window.dispatchEvent(new CustomEvent('toast', {
						detail: { type: 'success', message: '<?php echo esc_js( __( 'PDF exported successfully.', 'vms-theme' ) ); ?>' }
					}));
