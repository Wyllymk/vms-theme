...
<div class="flex flex-col gap-6" x-data="membersPage()" x-init="init()">
    ...
    <div class="vms-card">
        <div class="vms-flex vms-items-center vms-justify-between" style="flex-wrap:wrap;gap:1rem;">
            ...
            <div class="vms-flex vms-gap-2" style="flex-wrap:wrap;">
				...
				<!-- Total badge -->
				<span class="inline-flex items-center px-3 py-1 text-xs font-bold rounded-full bg-[var(--vms-primary)]/10 text-[var(--vms-primary)]" x-text="totalMembers + ' total'"></span>
            </div>
        </div>
    </div>
	...
	<div class="vms-card">
		<div class="flex flex-col gap-3 lg:flex-row">
			...
			<div class="relative flex-1">
				...
				<input
					type="search"
					x-model="searchTerm"
					@input.debounce.400ms="loadMembers()"
			...
			<select
				x-model="roleFilter"
				@change="currentPage = 1; loadMembers()"
		...
		<div class="flex flex-wrap gap-2 mt-3">
			<template x-for="tab in statusTabs" :key="tab.value">
				<button
					@click="statusFilter = tab.value; currentPage = 1; loadMembers()"
					class="px-3.5 py-1.5 text-sm font-medium rounded-lg transition-all duration-200"
	...
	<div class="overflow-hidden vms-card">
		...
		<div x-show="loading" class="text-center vms-card">
			...
			<p class="mt-3 text-sm text-gray-500 dark:text-gray-400"><?php esc_html_e( 'Loading members...', 'vms-theme' ); ?></p>
		</div>
			...
			<table class="w-full">
				<thead>
					<tr class="border-b border-gray-200 dark:border-gray-700">
						...
						<th class="hidden px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase dark:text-gray-400 md:table-cell">
							<?php esc_html_e( 'Email', 'vms-theme' ); ?>
						</th>
						<th class="hidden px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase dark:text-gray-400 lg:table-cell">
						...
						<th class="hidden px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase dark:text-gray-400 xl:table-cell">
						...
						<tr class="transition-colors hover:bg-gray-50 dark:hover:bg-gray-700/30">
							...
							<td class="px-6 py-4">
								<div class="flex items-center gap-3">
									<div class="w-10 h-10 rounded-full bg-[var(--vms-primary)]/10 flex items-center justify-center text-sm font-medium text-[var(--vms-primary)]"
										x-text="getInitials(member)"
									></div>
									<div class="min-w-0">
										<p class="text-sm font-medium text-gray-900 truncate dark:text-white" x-text="getFullName(member)"></p>
										<p class="text-xs text-gray-500 truncate dark:text-gray-400 md:hidden" x-text="member.email"></p>
									</div>
								</div>
							</td>
							<!-- Email -->
							<td class="hidden px-6 py-4 md:table-cell">
								<span class="text-sm text-gray-600 dark:text-gray-400" x-text="member.email || '-'"></span>
							</td>
							<!-- Phone -->
							<td class="hidden px-6 py-4 lg:table-cell">
								<span class="text-sm text-gray-600 dark:text-gray-400" x-text="member.phone || '-'"></span>
							</td>
							<!-- Role badge -->
							<td class="px-6 py-4">
								<span class="inline-flex px-2.5 py-0.5 text-xs font-medium rounded-full"
									:class="getRoleBadgeClass(member)"
									x-text="formatRole(member)"
								></span>
							</td>
							<!-- Status badge -->
							<td class="px-6 py-4">
								<span class="inline-flex px-2.5 py-0.5 text-xs font-medium rounded-full"
									:class="getStatusBadgeClass(member)"
									x-text="formatStatus(member)"
								></span>
							</td>
							<!-- Registered date -->
							<td class="hidden px-6 py-4 xl:table-cell">
								<span class="text-sm text-gray-500 dark:text-gray-400" x-text="formatDate(member.registered)"></span>
							</td>
							<!-- Actions -->
							<td class="px-6 py-4 text-right">
								<div class="flex items-center justify-end gap-1">
									...
									<button
										x-show="member.member_status === 'pending'"
		...
		<div x-show="!loading && members.length === 0" x-cloak class="text-center vms-card">
			...
			<p class="text-gray-500 dark:text-gray-400"
				x-text="searchTerm || statusFilter || roleFilter
					? '<?php echo esc_js( __( 'No members match your current filters. Try adjusting your search criteria.', 'vms-theme' ) ); ?>'
					: '<?php echo esc_js( __( 'There are no registered members yet.', 'vms-theme' ) ); ?>'"
			></p>
		</div>

		<!-- Pagination -->
		<div x-show="!loading && totalPages > 1" x-cloak class="flex flex-col items-center justify-between gap-3 px-6 py-4 border-t border-gray-200 sm:flex-row dark:border-gray-700">
			<p class="text-sm text-gray-500 dark:text-gray-400">
				<?php esc_html_e( 'Showing', 'vms-theme' ); ?>
				<span class="font-medium text-gray-900 dark:text-white" x-text="((currentPage - 1) * perPage) + 1"></span>
				<?php esc_html_e( 'to', 'vms-theme' ); ?>
				<span class="font-medium text-gray-900 dark:text-white" x-text="Math.min(currentPage * perPage, totalMembers)"></span>
			...
			<div class="flex items-center gap-1">
				...
				<template x-for="p in paginationRange()" :key="p">
					<button
						x-show="p !== '...'"
						@click="goToPage(p)"
						class="min-w-[36px] h-9 px-2 rounded-lg text-sm font-medium transition-all duration-200"
						:class="p === currentPage
							? 'bg-[var(--vms-primary)] text-white shadow-lg shadow-[var(--vms-primary)]/25'
					...
					<span
						x-show="p === '...'"
						class="min-w-[36px] h-9 flex items-center justify-center text-gray-400 text-sm"
					>...</span>
				</template>
	...
	>
		...
		<div
			class="relative w-full max-w-md bg-white border border-gray-200 shadow-2xl dark:bg-gray-800 rounded-2xl dark:border-gray-700"
			@click.stop
			x-transition:enter="transition ease-out duration-200"
		...
		>
			<!-- Top shine -->
			<div class="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white/60 to-transparent rounded-t-2xl" aria-hidden="true"></div>

			<div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
				<h3 class="text-lg font-semibold text-gray-900 dark:text-white">
					<?php esc_html_e( 'Reject Member', 'vms-theme' ); ?>
				</h3>
				<button @click="showRejectModal = false" class="p-1 text-gray-400 transition-colors rounded-lg hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700">
			...
			<div class="p-6">
				...
				<p class="mb-4 text-sm font-semibold text-gray-900 dark:text-white" x-text="rejectTarget ? getFullName(rejectTarget) : ''"></p>
				...
				<div class="mb-4">
					...
					<textarea
						...
						class="w-full px-3 py-2.5 bg-gray-50 dark:bg-gray-700/50 border border-gray-300 dark:border-gray-600 rounded-xl text-gray-900 dark:text-white placeholder-gray-400 focus:ring-2 focus:ring-[var(--vms-primary)] focus:border-transparent transition-colors resize-none"
					></textarea>
				</div>
		...
		<div class="overflow-y-auto vms-modal vms-modal-lg" @click.stop>
			...
			<form @submit.prevent="submitEditMember" class="p-6 space-y-4">
				<div class="grid grid-cols-2 gap-4">
					<div>
						<label class="block mb-1 text-sm font-medium text-gray-700 dark:text-gray-300"><?php esc_html_e( 'First Name', 'vms-theme' ); ?> <span class="text-red-500">*</span></label>
						<input type="text" x-model="editForm.first_name" required class="w-full px-3 py-2 bg-gray-50 dark:bg-gray-700/50 border border-gray-300 dark:border-gray-600 rounded-xl text-gray-900 dark:text-white focus:ring-2 focus:ring-[var(--vms-primary)]">
					</div>
					<div>
						<label class="block mb-1 text-sm font-medium text-gray-700 dark:text-gray-300"><?php esc_html_e( 'Last Name', 'vms-theme' ); ?> <span class="text-red-500">*</span></label>
						<input type="text" x-model="editForm.last_name" required class="w-full px-3 py-2 bg-gray-50 dark:bg-gray-700/50 border border-gray-300 dark:border-gray-600 rounded-xl text-gray-900 dark:text-white focus:ring-2 focus:ring-[var(--vms-primary)]">
					</div>
				</div>
				<div class="grid grid-cols-2 gap-4">
					<div>
						<label class="block mb-1 text-sm font-medium text-gray-700 dark:text-gray-300"><?php esc_html_e( 'Phone', 'vms-theme' ); ?></label>
						<input type="tel" x-model="editForm.phone" class="w-full px-3 py-2 bg-gray-50 dark:bg-gray-700/50 border border-gray-300 dark:border-gray-600 rounded-xl text-gray-900 dark:text-white focus:ring-2 focus:ring-[var(--vms-primary)]">
					</div>
					<div>
						<label class="block mb-1 text-sm font-medium text-gray-700 dark:text-gray-300"><?php esc_html_e( 'Email', 'vms-theme' ); ?></label>
						<input type="email" x-model="editForm.email" class="w-full px-3 py-2 bg-gray-50 dark:bg-gray-700/50 border border-gray-300 dark:border-gray-600 rounded-xl text-gray-900 dark:text-white focus:ring-2 focus:ring-[var(--vms-primary)]">
					</div>
				</div>
				<div class="grid grid-cols-2 gap-4">
					<div>
						<label class="block mb-1 text-sm font-medium text-gray-700 dark:text-gray-300"><?php esc_html_e( 'Member Number', 'vms-theme' ); ?></label>
						<input type="text" x-model="editForm.member_number" class="w-full px-3 py-2 bg-gray-50 dark:bg-gray-700/50 border border-gray-300 dark:border-gray-600 rounded-xl text-gray-900 dark:text-white focus:ring-2 focus:ring-[var(--vms-primary)]">
					</div>
					<div>
						<label class="block mb-1 text-sm font-medium text-gray-700 dark:text-gray-300"><?php esc_html_e( 'Status', 'vms-theme' ); ?></label>
						<select x-model="editForm.member_status" class="w-full px-3 py-2 bg-gray-50 dark:bg-gray-700/50 border border-gray-300 dark:border-gray-600 rounded-xl text-gray-900 dark:text-white focus:ring-2 focus:ring-[var(--vms-primary)]">
				...
				<div x-show="editError" x-cloak class="p-3 text-sm text-red-600 rounded-xl bg-red-50" x-text="editError"></div>
				<div class="flex justify-end gap-3 pt-2">
					<button type="button" @click="showEditModal = false" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-xl"><?php esc_html_e( 'Cancel', 'vms-theme' ); ?></button>
					<button type="submit" :disabled="editSaving" class="px-4 py-2 text-sm font-medium text-white bg-[var(--vms-primary)] rounded-xl flex items-center gap-2">
						<svg x-show="editSaving" x-cloak class="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg>
		...
		>
			...
			<div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700 shrink-0">
				<div class="flex items-center gap-3" x-show="detailMember">
					...
					<div>
						<h3 class="text-lg font-semibold text-gray-900 dark:text-white" x-text="detailMember ? getFullName(detailMember) : ''"></h3>
						<p class="text-xs text-gray-500 dark:text-gray-400" x-text="detailMember ? detailMember.email : ''"></p>
					</div>
				</div>
				<div class="flex items-center gap-2">
					...
					<button @click="showDetailModal = false" class="vms-btn vms-btn-sm vms-btn-secondary">&times;</button>
			...
			<div class="flex-1 p-6 space-y-6 overflow-y-auto">
				...
				<div x-show="detailLoading" class="py-8 text-center">
					...
					<p class="mt-2 text-sm text-gray-500 dark:text-gray-400"><?php esc_html_e( 'Loading member details...', 'vms-theme' ); ?></p>
				</div>
				...
				<div x-show="!detailLoading && detailMember" class="flex flex-col gap-3">
					...
					<div class="p-4 bg-gray-50 dark:bg-(--vms-glass-bg) border border-(--vms-border) rounded-xl">
						...
						<div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
							<div>
								<p class="text-xs text-gray-500 dark:text-gray-400"><?php esc_html_e( 'Full Name', 'vms-theme' ); ?></p>
								<p class="text-sm font-medium text-gray-900 dark:text-white" x-text="detailMember ? getFullName(detailMember) : '-'"></p>
							</div>
							<div>
								<p class="text-xs text-gray-500 dark:text-gray-400"><?php esc_html_e( 'Email', 'vms-theme' ); ?></p>
								<p class="text-sm font-medium text-gray-900 dark:text-white" x-text="detailMember ? (detailMember.email || '-') : '-'"></p>
							</div>
							<div>
								<p class="text-xs text-gray-500 dark:text-gray-400"><?php esc_html_e( 'Phone', 'vms-theme' ); ?></p>
								<p class="text-sm font-medium text-gray-900 dark:text-white" x-text="detailMember ? (detailMember.phone || '-') : '-'"></p>
							</div>
							<div>
								<p class="text-xs text-gray-500 dark:text-gray-400"><?php esc_html_e( 'Role', 'vms-theme' ); ?></p>
								<p class="text-sm font-medium text-gray-900 dark:text-white" x-text="detailMember ? formatRole(detailMember) : '-'"></p>
							</div>
							<div>
								<p class="text-xs text-gray-500 dark:text-gray-400"><?php esc_html_e( 'Status', 'vms-theme' ); ?></p>
								<span class="inline-flex px-2.5 py-0.5 text-xs font-medium rounded-full"
									:class="detailMember ? getStatusBadgeClass(detailMember) : ''"
									x-text="detailMember ? formatStatus(detailMember) : '-'"
								></span>
							</div>
							<div>
								<p class="text-xs text-gray-500 dark:text-gray-400"><?php esc_html_e( 'Registered', 'vms-theme' ); ?></p>
								<p class="text-sm font-medium text-gray-900 dark:text-white" x-text="detailMember ? formatDate(detailMember.registered) : '-'"></p>
							</div>
						</div>
					</div>
					...
					<div>
						<h4 class="flex items-center gap-2 mb-3 text-sm font-semibold text-gray-900 dark:text-white">
							<svg class="w-4 h-4 text-[var(--vms-primary)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
								<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/>
							</svg>
							<?php esc_html_e( 'Registered Guests', 'vms-theme' ); ?>
							<span class="text-xs font-normal text-gray-400" x-text="'(' + (detailGuests?.length || 0) + ')'"></span>
						</h4>

						<div x-show="detailGuests && detailGuests.length > 0" class="overflow-x-auto border border-gray-200 rounded-xl dark:border-gray-700">
							<table class="w-full">
								<thead>
									<tr class="bg-gray-50 dark:bg-gray-700/50">
										<th class="px-4 py-2 text-xs font-medium text-left text-gray-500 uppercase dark:text-gray-400"><?php esc_html_e( 'Guest Name', 'vms-theme' ); ?></th>
										<th class="hidden px-4 py-2 text-xs font-medium text-left text-gray-500 uppercase dark:text-gray-400 sm:table-cell"><?php esc_html_e( 'Phone', 'vms-theme' ); ?></th>
										<th class="px-4 py-2 text-xs font-medium text-left text-gray-500 uppercase dark:text-gray-400"><?php esc_html_e( 'Status', 'vms-theme' ); ?></th>
									</tr>
								</thead>
								<tbody class="divide-y divide-gray-100 dark:divide-gray-700/50">
									<template x-for="guest in detailGuests" :key="guest.id">
										<tr class="hover:bg-gray-50 dark:hover:bg-gray-700/20">
											<td class="px-4 py-2.5 text-sm text-gray-900 dark:text-white" x-text="(guest.first_name || '') + ' ' + (guest.last_name || '')"></td>
											<td class="px-4 py-2.5 text-sm text-gray-500 dark:text-gray-400 hidden sm:table-cell" x-text="guest.phone_number || '-'"></td>
						...
						<div x-show="!detailGuests || detailGuests.length === 0" class="py-6 text-center bg-gray-50 dark:bg-gray-700/20 rounded-xl">
							<p class="text-sm text-gray-400 dark:text-gray-500"><?php esc_html_e( 'No registered guests for this member.', 'vms-theme' ); ?></p>
						</div>
					</div>
					...
					<div>
						<h4 class="flex items-center gap-2 mb-3 text-sm font-semibold text-gray-900 dark:text-white">
							...
							<span class="text-xs font-normal text-gray-400" x-text="'(' + (detailVisits?.length || 0) + ')'"></span>
						</h4>

						<div x-show="detailVisits && detailVisits.length > 0" class="overflow-x-auto border border-gray-200 rounded-xl dark:border-gray-700">
							<table class="w-full">
								<thead>
									<tr class="bg-gray-50 dark:bg-gray-700/50">
										<th class="px-4 py-2 text-xs font-medium text-left text-gray-500 uppercase dark:text-gray-400"><?php esc_html_e( 'Guest', 'vms-theme' ); ?></th>
										<th class="hidden px-4 py-2 text-xs font-medium text-left text-gray-500 uppercase dark:text-gray-400 sm:table-cell"><?php esc_html_e( 'Date', 'vms-theme' ); ?></th>
										<th class="hidden px-4 py-2 text-xs font-medium text-left text-gray-500 uppercase dark:text-gray-400 md:table-cell"><?php esc_html_e( 'Sign In', 'vms-theme' ); ?></th>
										<th class="hidden px-4 py-2 text-xs font-medium text-left text-gray-500 uppercase dark:text-gray-400 md:table-cell"><?php esc_html_e( 'Sign Out', 'vms-theme' ); ?></th>
										<th class="px-4 py-2 text-xs font-medium text-left text-gray-500 uppercase dark:text-gray-400"><?php esc_html_e( 'Status', 'vms-theme' ); ?></th>
									</tr>
								</thead>
								<tbody class="divide-y divide-gray-100 dark:divide-gray-700/50">
									<template x-for="visit in detailVisits" :key="visit.id">
										<tr class="hover:bg-gray-50 dark:hover:bg-gray-700/20">
											<td class="px-4 py-2.5 text-sm text-gray-900 dark:text-white" x-text="(visit.first_name || '') + ' ' + (visit.last_name || '')"></td>
											<td class="px-4 py-2.5 text-sm text-gray-500 dark:text-gray-400 hidden sm:table-cell" x-text="visit.visit_date || '-'"></td>
											<td class="px-4 py-2.5 text-sm text-gray-500 dark:text-gray-400 hidden md:table-cell" x-text="visit.sign_in_time || '-'"></td>
											<td class="px-4 py-2.5 text-sm text-gray-500 dark:text-gray-400 hidden md:table-cell" x-text="visit.sign_out_time || '-'"></td>
											<td class="px-4 py-2.5">
												<span class="inline-flex px-2 py-0.5 text-xs font-medium rounded-full"
													:class="{
						...
						<div x-show="!detailVisits || detailVisits.length === 0" class="py-6 text-center bg-gray-50 dark:bg-gray-700/20 rounded-xl">
							<p class="text-sm text-gray-400 dark:text-gray-500"><?php esc_html_e( 'No visit history for this member.', 'vms-theme' ); ?></p>
						</div>
					</div>
				</div>
			</div>
			...
			<!-- Modal footer -->
			<div class="flex items-center justify-between px-6 py-4 border-t border-gray-200 shrink-0 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
				<button
					type="button"
					@click="detailMember && exportMemberPdf(detailMember)"
...
function membersPage() {
	return {
		...
		/* Status filter tabs */
		statusTabs: [
			{ value: '',           label: '<?php echo esc_js( __( 'All', 'vms-theme' ) ); ?>' },
			{ value: 'pending',    label: '<?php echo esc_js( __( 'Pending', 'vms-theme' ) ); ?>' },
			{ value: 'approved',   label: '<?php echo esc_js( __( 'Approved / Active', 'vms-theme' ) ); ?>' },
			{ value: 'suspended',  label: '<?php echo esc_js( __( 'Suspended', 'vms-theme' ) ); ?>' },
			{ value: 'banned',     label: '<?php echo esc_js( __( 'Banned', 'vms-theme' ) ); ?>' }
		],
		...
		async loadMembers() {
			...
			try {
				...
				if (data.success) {
					...
					this.totalMembers = 0;
					this.totalPages   = 0;
					window.vmsToast(data.data?.message || '<?php echo esc_js( __( 'Failed to load members.', 'vms-theme' ) ); ?>', 'error');
				}
			} catch (e) {
				console.error('Failed to load members:', e);
				window.vmsToast('<?php echo esc_js( __( 'An error occurred while loading members.', 'vms-theme' ) ); ?>', 'error');
			} finally {
				this.loading = false;
			}
		},
		...
		openApproveConfirm(member) {
			if (!confirm('<?php echo esc_js( __( 'Are you sure you want to approve this member?', 'vms-theme' ) ); ?>')) return;
		...
		async approveMember(member) {
			try {
				...
				if (data.success) {
					window.vmsToast(data.data?.message || '<?php echo esc_js( __( 'Member approved successfully.', 'vms-theme' ) ); ?>', 'success');
					this.loadMembers();
				} else {
					window.vmsToast(data.data?.message || '<?php echo esc_js( __( 'Failed to approve member.', 'vms-theme' ) ); ?>', 'error');
				}
			} catch (e) {
		...
		async confirmReject() {
			...
			try {
				...
				if (data.success) {
					window.vmsToast(data.data?.message || '<?php echo esc_js( __( 'Member rejected.', 'vms-theme' ) ); ?>', 'success');
					this.showRejectModal = false;
					this.rejectTarget = null;
					this.loadMembers();
				} else {
					window.vmsToast(data.data?.message || '<?php echo esc_js( __( 'Failed to reject member.', 'vms-theme' ) ); ?>', 'error');
				}
			} catch (e) {
		...
		async submitEditMember() {
			...
			try {
				...
				if (data.success) {
					...
					window.vmsToast(data.data?.message || '<?php echo esc_js( __( 'Member updated successfully.', 'vms-theme' ) ); ?>', 'success');
					this.loadMembers();
				} else {
					this.editError = data.data?.message || '<?php echo esc_js( __( 'Failed to update member.', 'vms-theme' ) ); ?>';
				}
			} catch (e) {
		...
		async updateMemberStatus(member, newStatus) {
			...
			try {
				...
				if (data.success) {
					...
					window.vmsToast(data.data?.message || '<?php echo esc_js( __( 'Member status updated.', 'vms-theme' ) ); ?>', 'success');
					this.loadMembers();
				} else {
					/* revert the dropdown */
					this.detailStatusChange = member.member_status;
					window.vmsToast(data.data?.message || '<?php echo esc_js( __( 'Failed to update status.', 'vms-theme' ) ); ?>', 'error');
				}
			} catch (e) {
				this.detailStatusChange = member.member_status;
		...
		async viewMember(member) {
			...
			try {
				...
				const [guestRes, visitRes] = await Promise.all([
					fetch(vmsTheme.ajaxUrl, { method: 'POST', credentials: 'same-origin', body: guestFd }),
					fetch(vmsTheme.ajaxUrl, { method: 'POST', credentials: 'same-origin', body: visitFd })
				]);
		...
		async exportAllMembers() {
			...
			try {
				...
				window.open(vmsTheme.ajaxUrl + '?' + params.toString(), '_blank');
				window.vmsToast('<?php echo esc_js( __( 'PDF export started. Check your downloads.', 'vms-theme' ) ); ?>', 'success');
			} catch (e) {
				console.error('Export failed:', e);
				window.vmsToast('<?php echo esc_js( __( 'Export failed.', 'vms-theme' ) ); ?>', 'error');
			} finally {
				/* Short delay so the user sees loading state */
				setTimeout(() => { this.exportingAll = false; }, 1500);
			}
		},
