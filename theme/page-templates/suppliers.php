...
<div class="space-y-6" x-data="suppliersPage()" x-init="init()">
	<!-- Page Header -->
	...
	<div class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-lg border border-gray-200/50 dark:border-gray-700/50 p-4">
		<div class="relative">
			...
			<input
				type="search"
				x-model="searchTerm"
				placeholder="<?php esc_attr_e( 'Search suppliers...', 'vms-theme' ); ?>"
	...
	<div x-show="!loading && filteredSuppliers.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
		<template x-for="supplier in filteredSuppliers" :key="supplier.id">
			<div class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-lg border border-gray-200/50 dark:border-gray-700/50 p-6 hover:shadow-xl transition-shadow">
				<div class="flex items-start justify-between mb-3">
					<div class="w-12 h-12 rounded-xl bg-[var(--vms-primary)]/10 flex items-center justify-center">
						<svg class="w-6 h-6 text-[var(--vms-primary)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
					...
					<span class="inline-flex px-2.5 py-0.5 text-xs font-medium rounded-full capitalize"
						...
						x-text="supplier.supplier_status || 'unknown'"
					></span>
				</div>
	...
	<div x-show="loading" class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-lg p-8 text-center">
		...
		<p class="mt-2 text-sm text-gray-500 dark:text-gray-400"><?php esc_html_e( 'Loading suppliers...', 'vms-theme' ); ?></p>
	</div>

	<!-- Empty State -->
	<div x-show="!loading && filteredSuppliers.length === 0" class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-lg p-12 text-center">
	...
	<div x-show="showAddModal" x-cloak class="fixed inset-0 z-[200] flex items-center justify-center p-4">
		...
		<div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 w-full max-w-lg" @click.stop>
			<div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
				<h3 class="text-lg font-semibold text-gray-900 dark:text-white" x-text="editingSupplier ? '<?php echo esc_js( __( 'Edit Supplier', 'vms-theme' ) ); ?>' : '<?php echo esc_js( __( 'Add Supplier', 'vms-theme' ) ); ?>'"></h3>
			...
			<form @submit.prevent="saveSupplier" class="p-6 space-y-4">
				<div>
					<label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"><?php esc_html_e( 'Company Name', 'vms-theme' ); ?> <span class="text-red-500">*</span></label>
				...
				<div class="grid grid-cols-2 gap-4">
					<div>
						<label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"><?php esc_html_e( 'Contact First Name', 'vms-theme' ); ?> <span class="text-red-500">*</span></label>
						<input type="text" x-model="supplierForm.contact_first_name" required
							class="w-full px-3 py-2 bg-gray-50 dark:bg-gray-700/50 border border-gray-300 dark:border-gray-600 rounded-xl text-gray-900 dark:text-white focus:ring-2 focus:ring-[var(--vms-primary)] focus:border-transparent">
					</div>
					<div>
						<label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"><?php esc_html_e( 'Contact Last Name', 'vms-theme' ); ?> <span class="text-red-500">*</span></label>
				...
				<div class="grid grid-cols-2 gap-4">
					<div>
						<label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"><?php esc_html_e( 'Phone', 'vms-theme' ); ?></label>
				...
				<div class="grid grid-cols-2 gap-4">
					<div>
						<label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"><?php esc_html_e( 'ID Number', 'vms-theme' ); ?></label>
						<input type="text" x-model="supplierForm.id_number"
							class="w-full px-3 py-2 bg-gray-50 dark:bg-gray-700/50 border border-gray-300 dark:border-gray-600 rounded-xl text-gray-900 dark:text-white focus:ring-2 focus:ring-[var(--vms-primary)] focus:border-transparent">
					</div>
					<div>
						<label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"><?php esc_html_e( 'Vehicle Registration', 'vms-theme' ); ?></label>
				...
				<div x-show="supplierError" x-cloak class="p-3 rounded-xl bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 text-sm text-red-600 dark:text-red-400" x-text="supplierError"></div>
...
function suppliersPage() {
	return {
		...
		saving: false,
		supplierError: '',
		supplierForm: { company_name: '', contact_first_name: '', contact_last_name: '', phone_number: '', email: '', id_number: '', vehicle_reg: '', status: 'active' },
		...
		async saveSupplier() {
			...
			try {
				...
				if (data.success) {
					...
					this.loadSuppliers();
					window.vmsToast(data.data?.message || '<?php echo esc_js( __( 'Supplier saved successfully.', 'vms-theme' ) ); ?>', 'success');
				} else {
		...
		async deleteSupplier(supplier) {
			...
			try {
				...
				if (data.success) {
					this.loadSuppliers();
					window.vmsToast(data.data?.message || '<?php echo esc_js( __( 'Supplier removed.', 'vms-theme' ) ); ?>', 'success');
				} else {
