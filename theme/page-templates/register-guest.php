...
<div class="max-w-3xl mx-auto flex flex-col gap-6" x-data="registerGuestPage()" x-init="init()">
    <!-- Page Header -->
    <div>
    ...
    <div
        class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-lg border border-gray-200/50 dark:border-gray-700/50 p-6">
        ...
        <div class="relative">
            ...
            <input type="search" x-model.debounce.400ms="searchTerm" @input="searchExisting()"
        ...
        <div x-show="searchResults.length > 0" x-cloak
            class="mt-3 border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden">
            <template x-for="result in searchResults" :key="result.id">
                <button @click="selectExistingGuest(result)"
                    ...
                    <div class="w-8 h-8 rounded-full bg-[var(--vms-primary)]/10 flex items-center justify-center text-xs font-medium text-[var(--vms-primary)]"
                        x-text="(result.first_name?.[0] || '') + (result.last_name?.[0] || '')"></div>
                    <div>
                    ...
                    <span class="ml-auto text-xs px-2 py-0.5 rounded-full" :class="{
						...
						}" x-text="result.guest_status"></span>
                </button>
            </template>
        </div>
    </div>
    ...
    <form @submit.prevent="handleSubmit" class="flex flex-col gap-6">
            ...
            <div class="flex items-center justify-between mb-4">
                ...
                <span x-show="selectedGuest" x-cloak class="text-sm text-[var(--vms-primary)] font-medium">
                    ...
                    <button type="button" @click="clearSelection()"
                        class="ml-1 underline"><?php esc_html_e( 'Clear', 'vms-theme' ); ?></button>
                </span>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div class="flex items-end gap-4">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" x-model="form.receive_emails"
                            class="w-4 h-4 rounded border-gray-300 dark:border-gray-600 text-[var(--vms-primary)] focus:ring-[var(--vms-primary)]">
                        <span
                            class="text-sm text-gray-700 dark:text-gray-300"><?php esc_html_e( 'Email notifications', 'vms-theme' ); ?></span>
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" x-model="form.receive_messages"
                            class="w-4 h-4 rounded border-gray-300 dark:border-gray-600 text-[var(--vms-primary)] focus:ring-[var(--vms-primary)]">
                        <span
                            class="text-sm text-gray-700 dark:text-gray-300"><?php esc_html_e( 'SMS notifications', 'vms-theme' ); ?></span>
                    </label>
                </div>
            </div>
        ...
        <div
            ...
            <div class="flex items-center gap-3 mb-4">
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" x-model="registerVisit"
                        class="w-4 h-4 rounded border-gray-300 dark:border-gray-600 text-[var(--vms-primary)] focus:ring-[var(--vms-primary)]">
                    <span
                        class="text-lg font-semibold text-gray-900 dark:text-white"><?php esc_html_e( 'Register Visit', 'vms-theme' ); ?></span>
                </label>
            </div>
        ...
        <div class="flex justify-end gap-3">
            ...
            <button type="submit" :disabled="loading"
                ...
                <span
                    x-text="loading ? '<?php echo esc_js( __( 'Registering...', 'vms-theme' ) ); ?>' : (registerVisit ? '<?php echo esc_js( __( 'Register Guest & Visit', 'vms-theme' ) ); ?>' : '<?php echo esc_js( __( 'Register Guest', 'vms-theme' ) ); ?>')"></span>
            </button>
        </div>
    </form>
</div>
