...
<div class="space-y-6" x-data="reportsPage()" x-init="init()">
	<!-- Page Header -->
	<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
		<div>
			...
			<p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
				<?php esc_html_e( 'Visitor statistics and trends.', 'vms-theme' ); ?>
			</p>
		</div>
	...
	<div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
		<div class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-lg border border-gray-200/50 dark:border-gray-700/50 p-5">
			<p class="text-sm font-medium text-gray-500 dark:text-gray-400"><?php esc_html_e( 'Total Visits', 'vms-theme' ); ?></p>
			<p class="mt-1 text-2xl font-bold text-gray-900 dark:text-white" x-text="reportStats.totalVisits">--</p>
		</div>
		<div class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-lg border border-gray-200/50 dark:border-gray-700/50 p-5">
			<p class="text-sm font-medium text-gray-500 dark:text-gray-400"><?php esc_html_e( 'Unique Guests', 'vms-theme' ); ?></p>
			<p class="mt-1 text-2xl font-bold text-gray-900 dark:text-white" x-text="reportStats.uniqueGuests">--</p>
		</div>
		<div class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-lg border border-gray-200/50 dark:border-gray-700/50 p-5">
			<p class="text-sm font-medium text-gray-500 dark:text-gray-400"><?php esc_html_e( 'Avg. Daily Visits', 'vms-theme' ); ?></p>
			<p class="mt-1 text-2xl font-bold text-gray-900 dark:text-white" x-text="reportStats.avgDaily">--</p>
		</div>
		<div class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-lg border border-gray-200/50 dark:border-gray-700/50 p-5">
			<p class="text-sm font-medium text-gray-500 dark:text-gray-400"><?php esc_html_e( 'Completion Rate', 'vms-theme' ); ?></p>
	...
	<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
		...
		<div class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-lg border border-gray-200/50 dark:border-gray-700/50 p-6">
			...
			<div class="relative h-64">
				...
				<div x-show="loading" class="absolute inset-0 flex items-center justify-center bg-white/50 dark:bg-gray-800/50 rounded-xl">
			...
			<table class="w-full">
				<thead>
					<tr class="border-b border-gray-200 dark:border-gray-700">
						<th class="text-left px-4 py-2 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase"><?php esc_html_e( 'Guest', 'vms-theme' ); ?></th>
						<th class="text-left px-4 py-2 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase"><?php esc_html_e( 'Visits', 'vms-theme' ); ?></th>
						<th class="text-left px-4 py-2 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase hidden sm:table-cell"><?php esc_html_e( 'Last Visit', 'vms-theme' ); ?></th>
					</tr>
				</thead>
				<tbody class="divide-y divide-gray-100 dark:divide-gray-700/50">
					<template x-for="guest in topGuests" :key="guest.id">
						<tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30">
							<td class="px-4 py-3">
								<span class="text-sm font-medium text-gray-900 dark:text-white" x-text="guest.name"></span>
							</td>
							<td class="px-4 py-3">
								<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-[var(--vms-primary)]/10 text-[var(--vms-primary)]" x-text="guest.count"></span>
							</td>
...
function reportsPage() {
	return {
		dateRange: '30',
		loading: true,
		reportStats: { totalVisits: '--', uniqueGuests: '--', avgDaily: '--', completionRate: '--' },
		topGuests: [],
		dailyChart: null,
		statusChart: null,
		...
		processReportData(visits) {
			...
			visits.forEach(v => {
				...
				if (!guestMap[key]) {
					guestMap[key] = { id: key, name: v.first_name + ' ' + v.last_name, count: 0, lastVisit: v.visit_date };
				}
				guestMap[key].count++;
			});
		...
		renderDailyChart(visits) {
			...
			const primary = getComputedStyle(document.documentElement).getPropertyValue('--vms-primary').trim() || '#0ea5e9';
		...
		renderStatusChart(visits) {
			...
			const statusCounts = { approved: 0, completed: 0, unapproved: 0, cancelled: 0 };
			visits.forEach(v => {
			...
			this.statusChart = new Chart(ctx, {
				...
				data: {
					...
					datasets: [{
						data: [statusCounts.approved, statusCounts.completed, statusCounts.unapproved, statusCounts.cancelled],
		...
		exportData() {
			window.dispatchEvent(new CustomEvent('toast', {
				detail: { type: 'info', message: '<?php echo esc_js( __( 'Export feature coming soon.', 'vms-theme' ) ); ?>' }
			}));
		}
	};
}
