...
<div class="space-y-6" x-data="staffPage()" x-init="init()">
	...
	<div class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-lg border border-gray-200/50 dark:border-gray-700/50 p-4 space-y-4">
		<!-- Status Tabs -->
		<div class="flex items-center gap-1 border-b border-gray-200 dark:border-gray-700 pb-3 overflow-x-auto">
			<template x-for="tab in statusTabs" :key="tab.value">
				<button
					@click="statusFilter = tab.value; currentPage = 1; fetchStaff()"
					...
					x-text="tab.label + (tab.value === '' ? ' (' + totalRecords + ')' : '')"
				></button>
			</template>
		</div>
		...
		<div class="flex flex-col sm:flex-row gap-3">
			<div class="relative flex-1">
				...
				<input
					type="search"
					x-model="searchTerm"
					@input.debounce.400ms="currentPage = 1; fetchStaff()"
			...
			<select
				x-model="roleFilter"
				@change="currentPage = 1; fetchStaff()"
	...
	<div class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-lg border border-gray-200/50 dark:border-gray-700/50 overflow-hidden">
		...
		<div x-show="loading" class="p-8 text-center">
			...
			<p class="mt-2 text-sm text-gray-500 dark:text-gray-400"><?php esc_html_e( 'Loading staff...', 'vms-theme' ); ?></p>
		</div>
			...
			<table class="w-full">
				<thead>
					<tr class="border-b border-gray-200 dark:border-gray-700">
						...
						<th class="text-left px-6 py-3 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider hidden sm:table-cell">
							<?php esc_html_e( 'Employee #', 'vms-theme' ); ?>
						</th>
						<th class="text-left px-6 py-3 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider hidden lg:table-cell">
							<?php esc_html_e( 'Email', 'vms-theme' ); ?>
						</th>
						<th class="text-left px-6 py-3 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider hidden lg:table-cell">
							<?php esc_html_e( 'Phone', 'vms-theme' ); ?>
						</th>
						<th class="text-left px-6 py-3 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider hidden md:table-cell">
						...
						<th class="text-left px-6 py-3 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider hidden xl:table-cell">
							<?php esc_html_e( 'Hire Date', 'vms-theme' ); ?>
						</th>
						<th class="text-right px-6 py-3 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
					...
					<template x-for="staff in paginatedStaff" :key="staff.id">
						<tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
							...
							<td class="px-6 py-4">
								<div class="flex items-center gap-3">
									<div class="w-10 h-10 rounded-full bg-[var(--vms-primary)]/10 flex items-center justify-center text-sm font-semibold text-[var(--vms-primary)] shrink-0"
										x-text="(staff.first_name?.[0] || '') + (staff.last_name?.[0] || '')"
									></div>
									<div class="min-w-0">
										<p class="text-sm font-medium text-gray-900 dark:text-white truncate" x-text="staff.first_name + ' ' + staff.last_name"></p>
										<p class="text-xs text-gray-500 dark:text-gray-400 sm:hidden" x-text="'#' + (staff.employee_number || '-')"></p>
									</div>
								</div>
							</td>
							...
							<td class="px-6 py-4 hidden sm:table-cell">
								<span class="text-sm text-gray-600 dark:text-gray-400 font-mono" x-text="staff.employee_number || '-'"></span>
							</td>

							<!-- Email -->
							<td class="px-6 py-4 hidden lg:table-cell">
								<span class="text-sm text-gray-600 dark:text-gray-400" x-text="staff.email || '-'"></span>
							</td>

							<!-- Phone -->
							<td class="px-6 py-4 hidden lg:table-cell">
								<span class="text-sm text-gray-600 dark:text-gray-400" x-text="staff.phone_number || '-'"></span>
							</td>

							<!-- Department -->
							<td class="px-6 py-4 hidden md:table-cell">
								<span class="text-sm text-gray-600 dark:text-gray-400" x-text="staff.department || '-'"></span>
							</td>

							<!-- Position / Role -->
							<td class="px-6 py-4 hidden md:table-cell">
								<span class="inline-flex px-2.5 py-0.5 text-xs font-medium rounded-full bg-[var(--vms-primary)]/10 text-[var(--vms-primary)]"
									x-text="formatPosition(staff.position)"
								></span>
							</td>
							...
							<td class="px-6 py-4">
								<span class="inline-flex px-2.5 py-0.5 text-xs font-medium rounded-full capitalize"
									...
									x-text="staff.employee_status || 'unknown'"
								></span>
							</td>

							<!-- Hire Date -->
							<td class="px-6 py-4 hidden xl:table-cell">
								<span class="text-sm text-gray-600 dark:text-gray-400" x-text="staff.hire_date ? formatDate(staff.hire_date) : '-'"></span>
							</td>
		...
		<div x-show="!loading && paginatedStaff.length === 0" x-cloak class="p-12 text-center">
			...
			<p class="text-gray-500 dark:text-gray-400 mb-4"
				x-text="searchTerm || statusFilter || roleFilter
					...
					: '<?php echo esc_js( __( 'Register your first staff member to get started.', 'vms-theme' ) ); ?>'"
			></p>
			<button
				x-show="!searchTerm && !statusFilter && !roleFilter"
				@click="openRegisterModal()"
		...
		<!-- Pagination -->
		<div x-show="!loading && totalPages > 1" class="flex flex-col sm:flex-row items-center justify-between gap-3 px-6 py-4 border-t border-gray-200 dark:border-gray-700">
			<p class="text-sm text-gray-500 dark:text-gray-400">
				...
				<span class="font-medium text-gray-900 dark:text-white" x-text="Math.min(currentPage * perPage, totalRecords)"></span>
			...
			<div class="flex items-center gap-1">
				<button
					...
					class="px-2.5 py-1.5 text-sm font-medium rounded-lg border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
				>&laquo;</button>
				<button
	...
	>
		...
		<div
			class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 w-full max-w-2xl max-h-[90vh] flex flex-col"
			@click.stop
			x-transition:enter="transition ease-out duration-200"
		...
		>
			...
			<div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700 shrink-0">
				...
				<button @click="showRegisterModal = false" class="p-1 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
					<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
			...
			<!-- Body -->
			<form @submit.prevent="registerStaff()" class="p-6 space-y-4 overflow-y-auto">
	...
	>
		...
		>
			...
			<div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700 shrink-0">
				...
				<button @click="showEditModal = false" class="p-1 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
					<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
						<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
					</svg>
				</button>
			...
			<!-- Body -->
			<form @submit.prevent="updateStaff()" class="p-6 space-y-4 overflow-y-auto">
				<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
					<div>
						<label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
							<?php esc_html_e( 'First Name', 'vms-theme' ); ?> <span class="text-red-500">*</span>
	...
	>
		...
		<div
			class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 w-full max-w-3xl max-h-[90vh] flex flex-col"
			@click.stop
			x-transition:enter="transition ease-out duration-200"
			x-transition:enter-start="opacity-0 scale-95"
			x-transition:enter-end="opacity-100 scale-100"
		...
		>
			...
			<div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700 shrink-0">
				<div class="flex items-center gap-3">
					...
					<div>
						...
						></h3>
						<p class="text-sm text-gray-500 dark:text-gray-400"
							x-text="viewingStaff ? formatPosition(viewingStaff.position) : ''"
						></p>
					</div>
				</div>
				<div class="flex items-center gap-2">
					<button
						@click="viewingStaff && exportStaffPdf(viewingStaff)"
						class="p-1.5 rounded-lg text-gray-400 hover:text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-colors"
						title="<?php esc_attr_e( 'Export PDF', 'vms-theme' ); ?>"
					>
						<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
					...
					<button @click="showViewModal = false" class="p-1 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
						<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
						</svg>
					</button>
			...
			<div class="overflow-y-auto p-6 space-y-6">
				...
				<div class="flex items-center gap-1 border-b border-gray-200 dark:border-gray-700">
					...
					><?php esc_html_e( 'Details', 'vms-theme' ); ?></button>
					<button
						@click="viewTab = 'guests'"
						class="px-4 py-2.5 text-sm font-medium border-b-2 transition-colors"
						:class="viewTab === 'guests'
							? 'border-[var(--vms-primary)] text-[var(--vms-primary)]'
							: 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'"
					><?php esc_html_e( 'Guests', 'vms-theme' ); ?></button>
					<button
						@click="viewTab = 'activity'"
						class="px-4 py-2.5 text-sm font-medium border-b-2 transition-colors"
						:class="viewTab === 'activity'
							? 'border-[var(--vms-primary)] text-[var(--vms-primary)]'
							: 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'"
					><?php esc_html_e( 'Activity Log', 'vms-theme' ); ?></button>
				</div>
				...
				<div x-show="viewTab === 'info'" class="space-y-4">
					<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
						<div class="bg-gray-50 dark:bg-gray-700/30 rounded-xl p-4">
							<p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1"><?php esc_html_e( 'Full Name', 'vms-theme' ); ?></p>
							<p class="text-sm font-medium text-gray-900 dark:text-white" x-text="viewingStaff ? viewingStaff.first_name + ' ' + viewingStaff.last_name : '-'"></p>
						</div>
						<div class="bg-gray-50 dark:bg-gray-700/30 rounded-xl p-4">
							<p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1"><?php esc_html_e( 'Employee Number', 'vms-theme' ); ?></p>
							<p class="text-sm font-medium text-gray-900 dark:text-white font-mono" x-text="viewingStaff?.employee_number || '-'"></p>
						</div>
						<div class="bg-gray-50 dark:bg-gray-700/30 rounded-xl p-4">
							<p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1"><?php esc_html_e( 'Email', 'vms-theme' ); ?></p>
							<p class="text-sm font-medium text-gray-900 dark:text-white" x-text="viewingStaff?.email || '-'"></p>
						</div>
						<div class="bg-gray-50 dark:bg-gray-700/30 rounded-xl p-4">
							<p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1"><?php esc_html_e( 'Phone', 'vms-theme' ); ?></p>
							<p class="text-sm font-medium text-gray-900 dark:text-white" x-text="viewingStaff?.phone_number || '-'"></p>
						</div>
						<div class="bg-gray-50 dark:bg-gray-700/30 rounded-xl p-4">
							<p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1"><?php esc_html_e( 'ID Number', 'vms-theme' ); ?></p>
							<p class="text-sm font-medium text-gray-900 dark:text-white" x-text="viewingStaff?.id_number || '-'"></p>
						</div>
						<div class="bg-gray-50 dark:bg-gray-700/30 rounded-xl p-4">
							<p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1"><?php esc_html_e( 'Department', 'vms-theme' ); ?></p>
							<p class="text-sm font-medium text-gray-900 dark:text-white" x-text="viewingStaff?.department || '-'"></p>
						</div>
						<div class="bg-gray-50 dark:bg-gray-700/30 rounded-xl p-4">
							<p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1"><?php esc_html_e( 'Position / Role', 'vms-theme' ); ?></p>
							<p class="text-sm font-medium text-gray-900 dark:text-white" x-text="viewingStaff ? formatPosition(viewingStaff.position) : '-'"></p>
						</div>
						<div class="bg-gray-50 dark:bg-gray-700/30 rounded-xl p-4">
							<p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1"><?php esc_html_e( 'Status', 'vms-theme' ); ?></p>
							<span class="inline-flex px-2.5 py-0.5 text-xs font-medium rounded-full capitalize"
								:class="{
								...
								x-text="viewingStaff?.employee_status || '-'"
							></span>
						</div>
						<div class="bg-gray-50 dark:bg-gray-700/30 rounded-xl p-4">
							<p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1"><?php esc_html_e( 'Hire Date', 'vms-theme' ); ?></p>
							<p class="text-sm font-medium text-gray-900 dark:text-white" x-text="viewingStaff?.hire_date ? formatDate(viewingStaff.hire_date) : '-'"></p>
						</div>
						<div class="bg-gray-50 dark:bg-gray-700/30 rounded-xl p-4">
							<p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1"><?php esc_html_e( 'WordPress User ID', 'vms-theme' ); ?></p>
							<p class="text-sm font-medium text-gray-900 dark:text-white" x-text="viewingStaff?.wp_user_id || '-'"></p>
						</div>
					</div>
				</div>

				<!-- Tab: Guests (signed in / registered by this staff member) -->
				<div x-show="viewTab === 'guests'" class="space-y-3">
					<div x-show="viewLoadingGuests" class="py-8 text-center">
						<svg class="animate-spin h-6 w-6 mx-auto text-[var(--vms-primary)]" fill="none" viewBox="0 0 24 24">
							<circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
							<path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path>
						</svg>
						<p class="mt-2 text-sm text-gray-500 dark:text-gray-400"><?php esc_html_e( 'Loading guests...', 'vms-theme' ); ?></p>
					</div>
					...
					<div x-show="!viewLoadingGuests && viewGuests.length === 0" class="py-8 text-center">
						...
						<p class="text-sm text-gray-500 dark:text-gray-400"><?php esc_html_e( 'No guests registered by this staff member yet.', 'vms-theme' ); ?></p>
					</div>
					...
					<template x-for="guest in viewGuests" :key="guest.id">
						<div class="flex items-center justify-between bg-gray-50 dark:bg-gray-700/30 rounded-xl p-3">
							<div class="flex items-center gap-3">
								...
								></div>
								<div>
									<p class="text-sm font-medium text-gray-900 dark:text-white" x-text="guest.first_name + ' ' + guest.last_name"></p>
									<p class="text-xs text-gray-500 dark:text-gray-400" x-text="guest.phone_number || guest.email || ''"></p>
								</div>
							</div>
							<span class="text-xs text-gray-400 dark:text-gray-500" x-text="guest.visit_date ? formatDate(guest.visit_date) : ''"></span>
						</div>
					</template>
				</div>
				...
				<div x-show="viewTab === 'activity'" class="space-y-3">
					<div x-show="viewLoadingActivity" class="py-8 text-center">
						...
						<p class="mt-2 text-sm text-gray-500 dark:text-gray-400"><?php esc_html_e( 'Loading activity...', 'vms-theme' ); ?></p>
					</div>
					...
					<div x-show="!viewLoadingActivity && viewActivity.length === 0" class="py-8 text-center">
						...
						<p class="text-sm text-gray-500 dark:text-gray-400"><?php esc_html_e( 'No activity recorded for this staff member yet.', 'vms-theme' ); ?></p>
					</div>
					...
					<template x-for="(entry, idx) in viewActivity" :key="idx">
						<div class="flex items-start gap-3 bg-gray-50 dark:bg-gray-700/30 rounded-xl p-3">
							...
							<div class="min-w-0 flex-1">
								...
								<p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5" x-text="entry.date ? formatDate(entry.date) : ''"></p>
							</div>
						</div>
					</template>
				</div>
			...
			<div class="flex justify-end gap-3 px-6 py-4 border-t border-gray-200 dark:border-gray-700 shrink-0">
				<button
					@click="viewingStaff && openEditModal(viewingStaff); showViewModal = false"
...
function staffPage() {
	return {
		...
		statusTabs: [
			...
			{ value: 'active', label: '<?php echo esc_js( __( 'Active', 'vms-theme' ) ); ?>' },
			{ value: 'suspended', label: '<?php echo esc_js( __( 'Suspended', 'vms-theme' ) ); ?>' },
			{ value: 'terminated', label: '<?php echo esc_js( __( 'Terminated', 'vms-theme' ) ); ?>' }
		],

		/* Register modal */
		showRegisterModal: false,
		registerForm: { first_name: '', last_name: '', email: '', phone_number: '', employee_number: '', id_number: '', department: '', position: '', hire_date: '' },
		registerError: '',
		...
		/* ── Register Staff ──────────────────────────────── */
		openRegisterModal() {
			this.registerForm = { first_name: '', last_name: '', email: '', phone_number: '', employee_number: '', id_number: '', department: '', position: '', hire_date: '' };
			this.registerError = '';
		...
		async registerStaff() {
			...
			try {
				...
				if (data.success) {
					...
					this.fetchStaff();
					window.vmsToast?.(data.data?.message || '<?php echo esc_js( __( 'Staff member registered successfully.', 'vms-theme' ) ); ?>', 'success');
				} else {
					this.registerError = data.data?.message || '<?php echo esc_js( __( 'Registration failed. Please try again.', 'vms-theme' ) ); ?>';
				}
			} catch (e) {
		...
		async updateStaff() {
			...
			try {
				...
				if (data.success) {
					...
					this.fetchStaff();
					window.vmsToast?.(data.data?.message || '<?php echo esc_js( __( 'Staff member updated successfully.', 'vms-theme' ) ); ?>', 'success');
				} else {
					this.editError = data.data?.message || '<?php echo esc_js( __( 'Update failed. Please try again.', 'vms-theme' ) ); ?>';
				}
			} catch (e) {
		...
		async confirmDelete(staff) {
			if (!confirm('<?php echo esc_js( __( 'Are you sure you want to delete this staff member? This action cannot be undone.', 'vms-theme' ) ); ?>')) return;
			try {
				const data = await this.ajaxPost('vms_delete_employee', { employee_id: staff.id });
				if (data.success) {
					this.fetchStaff();
					window.vmsToast?.(data.data?.message || '<?php echo esc_js( __( 'Staff member deleted.', 'vms-theme' ) ); ?>', 'success');
				} else {
					window.vmsToast?.(data.data?.message || '<?php echo esc_js( __( 'Delete failed.', 'vms-theme' ) ); ?>', 'error');
				}
			} catch (e) {
				window.vmsToast?.('<?php echo esc_js( __( 'An unexpected error occurred.', 'vms-theme' ) ); ?>', 'error');
			}
		},
		...
		async loadStaffGuests(staff) {
			...
			try {
				const data = await this.ajaxPost('vms_search_employees', { employee_id: staff.id, context: 'guests' });
				if (data.success) {
		...
		async loadStaffActivity(staff) {
			...
			try {
				const data = await this.ajaxPost('vms_search_employees', { employee_id: staff.id, context: 'activity' });
				if (data.success) {
		...
		async exportStaffPdf(staff) {
			try {
				const data = await this.ajaxPost('vms_export_staff_pdf', { employee_id: staff.id });
				if (data.success && data.data?.url) {
					window.open(data.data.url, '_blank');
					window.vmsToast?.('<?php echo esc_js( __( 'PDF exported successfully.', 'vms-theme' ) ); ?>', 'success');
				} else {
					window.vmsToast?.(data.data?.message || '<?php echo esc_js( __( 'PDF export failed.', 'vms-theme' ) ); ?>', 'error');
				}
			} catch (e) {
				window.vmsToast?.('<?php echo esc_js( __( 'An unexpected error occurred.', 'vms-theme' ) ); ?>', 'error');
			}
		...
		async exportAllStaffPdf() {
			this.exporting = true;
			try {
				const data = await this.ajaxPost('vms_export_staff_pdf', { export_all: 1 });
				if (data.success && data.data?.url) {
					window.open(data.data.url, '_blank');
					window.vmsToast?.('<?php echo esc_js( __( 'Staff list exported successfully.', 'vms-theme' ) ); ?>', 'success');
				} else {
					window.vmsToast?.(data.data?.message || '<?php echo esc_js( __( 'Export failed.', 'vms-theme' ) ); ?>', 'error');
				}
			} catch (e) {
				window.vmsToast?.('<?php echo esc_js( __( 'An unexpected error occurred.', 'vms-theme' ) ); ?>', 'error');
			} finally {
