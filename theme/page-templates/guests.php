...
<div class="vms-guests-page" x-data="guestsPage()" x-init="init()">
    ...
    <div class="vms-card" style="margin-bottom:1.5rem;padding:0.75rem 1rem;">
        <div class="vms-flex vms-items-center vms-gap-2" style="flex-wrap:wrap;">
            <button class="vms-btn vms-btn-sm" :class="statusFilter === '' ? 'vms-btn-primary' : 'vms-btn-secondary py-[0.615rem]! px-3.5!'"
    ...
    return {
        ...
        get paginatedGuests() {
            ...
            if (this.currentPage > this.totalPages) {
                console.warn(`[paginatedGuests] currentPage (${this.currentPage}) exceeds totalPages (${this.totalPages}), resetting to ${this.totalPages}`);
                this.currentPage = this.totalPages;
            }

            const start = (this.currentPage - 1) * this.perPage;
            const page = filtered.slice(start, start + this.perPage);
            console.log(`[paginatedGuests] Page ${this.currentPage}/${this.totalPages} — showing records ${start + 1}–${start + page.length} of ${this.totalRecords}`);
            return page;
        },
        ...
        async submitRegisterGuest() {
            console.log('[submitRegisterGuest] Submitting — form:', this.registerForm, '| selectedExistingGuest:', this.selectedExistingGuest);
            this.registerError = '';
            ...
            try {
                ...
                if (!guestId) {
                    ...
                    if (!guestJson.success) {
                        ...
                        if (guestJson.data?.code === 'duplicate_phone' && guestJson.data?.data?.existing_id) {
                            ...
                            console.warn('[submitRegisterGuest] Duplicate phone — reusing existing guest ID:', guestId);
                        } else {
                ...
                this.showToast('<?php echo esc_js( __( 'Guest registered successfully.', 'vms-theme' ) ); ?>', 'success');
                this.fetchGuests();
            } catch (e) {
        ...
        async submitEditGuest() {
            console.log('[submitEditGuest] Submitting edit for guest ID:', this.editingGuest?.id, '| form:', this.editForm);
            this.editError = '';
        ...
        /* ── Export PDF ──────────────────────────────────────────── */
        async exportPDF() {
            console.log('[exportPDF] Starting export — statusFilter:', this.statusFilter || '(none)', '| searchTerm:', this.searchTerm || '(none)');
            this.exporting = true;
            try {
                const formData = new FormData();
                ...
                if (contentType.includes('application/pdf') || contentType.includes('application/octet-stream')) {
                    ...
                    a.remove();
                    this.showToast('<?php echo esc_js( __( 'PDF exported successfully.', 'vms-theme' ) ); ?>', 'success');
                } else {
